...
export default function PlayerControls({
    ...
    return (
        <div className="bg-bg-card border-t border-border p-4 flex flex-col gap-4 w-full">
            {hasError && (
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        >
                            <div className="flex items-center gap-2">
                                {isLoadingSubtitles && <Loader2 size={14} className="animate-spin text-purple" />}
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 bottom-full mb-2 w-64 bg-bg border border-border rounded-xl shadow-lg z-50 overflow-hidden">
                                <div className="max-h-60 overflow-y-auto py-1">
                                    <button
                                        ...
                                        className="flex items-center justify-between w-full px-4 py-2 text-left text-sm text-text-primary hover:bg-bg-hover transition-colors"
                                    >
                                        <span>Altyazı Yok</span>
                                        {activeSubtitleId === null && <Check size={16} className="text-purple" />}
                                    </button>
                                    {subtitles.map(sub => (
                                        <button
                                            key={sub.id}
                                            ...
                                            className="flex items-center justify-between w-full px-4 py-2.5 text-left text-sm text-text-primary hover:bg-bg-hover transition-colors"
                                        >
                                            ...
                                            {activeSubtitleId === sub.id && <Check size={16} className="text-purple" />}
                                        </button>
                                    ))}
                                </div>
                            </div>
